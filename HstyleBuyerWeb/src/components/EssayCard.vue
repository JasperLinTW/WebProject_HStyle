<template>
  <!-- 内文解碼 -->
 
      <div class="col-md-3  mb-4">
        <div class="card border-0 card1">
          <div class="card-img w-100 h200px rounded overflow-hidden">

            <img :src="data.imgs[0]" class="card-img-top" alt="Essays Image">

          </div>

          <div class="card-header d-flex bg-white border-bottom-0">
            <span class="badge bg-secondary opacity-50 me-1" v-for="tag in data.tags">{{ tag }}</span>
        
          </div>
          <div class="card-body">
       
            <router-link :to="'/EssaysBlog/' + data.essayId" class="text-dark text-decoration-none stretched-link">{{
              data.etitle }}</router-link>

          </div>
          <div class="card-footer bg-white border-top-0 d-flex">

            <span class="me-auto">{{ data.uplodTime.slice(0, 10) }}</span>

            <!-- <span><i class="fa-regular fa-bookmark"></i></span> -->
            <div class="card-text text-end">
              <span v-if="!data.isClicked" @click="data.isClicked = true"><i
                  class="fa-regular fa-bookmark icon-hover fz-18"></i></span>
              <span v-else @click="data.isClicked = false"><i class="fa-solid fa-bookmark SolidHeart fz-18"></i></span>
            </div>
          </div>
        </div>
      </div>
</template>
<script setup>
import { ref, onMounted } from "vue";
import axios from "axios";
const props = defineProps({
  data: Object,
});
onMounted(() => {
  // getEssayInfo();
});

//收藏
let likes = ref([])

const Essays = ref([])



const likesEssay = async () => {
  await axios.get("https://localhost:7243/api/Essay/Elike")
    .then(response => {
      if (response.data.length > 0) {
        likes.value = response.data;
        console.log(likes.value);
        Essays.value = likes.value.map(e => {
          return e.essayId
        });
      }

    })
    .catch(error => { console.log(error); });
}


</script>
<style scoped>
.h200px {
  height: 200px;
}

.object-fit-cover {
  object-fit: cover;
}

.card1 img {
  transition: all 0.2s;
}

.card1:hover img {
  transform: scale(1.1);
}

.SolidHeart {
  color: #46a3ff;
}

.fz-18 {
  font-size: 20px;
  cursor: pointer;
}
</style>